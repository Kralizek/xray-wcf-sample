    ...
    public class StorageResourceAccessServiceHostConfigurator : IServiceHostConfigurator<StorageResourceAccess>
    {
        ...
        public void Configure(IServiceHost<StorageResourceAccess> host)
        {
            ...
            */
            ...
            //host.AddBasicHttpEndpoint(typeof(IStorageResourceAccess), 9999, hostname: _options.AnnouncedHost, configureBinding: binding => binding.UseDefaults());

            /* Adds a WSHttp endpoint on port 9999 with suffix: "ws". Uses default configuration. */
            //host.AddWSHttpEndpoint(typeof(IStorageResourceAccess), 9999, hostname: _options.AnnouncedHost, configureBinding: binding => binding.UseDefaults());

            /* Adds a WebHttp endpoint on port 9999 with suffix: "web". Uses default configuration. */
            //host.AddWebHttpEndpoint(typeof(IStorageResourceAccess), 9999, hostname: _options.AnnouncedHost, configureBinding: binding => binding.UseDefaults());

            /* Adds a NamedPipe endpoint with suffix: "echo". Uses default configuration. */
            //host.AddNamedPipeEndpoint(typeof(IStorageResourceAccess), "echo", configureBinding: binding => binding.UseDefaults());

            /* Adds a NetTcp endpoint on port 9998. Uses default configuration. */
            host.AddNetTcpEndpoint(typeof(IStorageResourceAccess), 9998, hostname: _options.AnnouncedHostName, configureBinding: binding => binding.UseDefaults());
